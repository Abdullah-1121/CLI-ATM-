...
// Functions for users registration 
// Defining interface for user data
interface userData{
    userName:string,
    userPin : number,
    balance :number
...
// function to load user data from the file
 ...
 function RegisterUser(Uname:string , Pin:number , Balance :number):void{
 ...
 async function deposit( User:userData[] , UsName:string){
...
async function WithDraw(User:userData[] , UsName:string){
    let userInput = await inquirer.prompt([{
        name : 'amount',
        type : 'number',
 ...
 async function Transfer (User:userData[] , UsName:string){
    let userInput = await inquirer.prompt([{
        name : 'Name',
        type : 'input',
...
if(enter.LOS=='LogIn'){
    ...
    LoaduserData();
    let user =
    User.find(user=>user.userName==userInput.Username);
